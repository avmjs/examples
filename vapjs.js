...
var VapQuery = __webpack_require__(16);
var VapFilter = __webpack_require__(6);
var VapContract = __webpack_require__(13);
...
function Vap(cprovider, options) {
  ...
  self.setProvider = function (provider) {
    var query = new VapQuery(provider, self.options.query);
...
var VapFilter = __webpack_require__(6); // eslint-disable-line
...
function HttpProvider(host, timeout) {
  if (!(this instanceof HttpProvider)) {
    throw new Error('[vapjs-provider-http] the HttpProvider instance requires the "new" flag in order to function normally (e.g. `const eth = new Vap(new HttpProvider());`).');
  }
...
function Vap(provider, options) {
  ...
  if (!(this instanceof Vap)) {
    throw new Error('[vapjs-query] the Vap object requires the "new" flag in order to function normally (i.e. `const eth = new Vap(provider);`).');
  }
  if (typeof provider !== 'object') {
    throw new Error('[vapjs-query] the Vap object requires that the first input \'provider\' must be an object, got \'' + typeof provider + '\' (i.e. \'const eth = new Vap(provider);\')');
  }
...
Vap.prototype.sendAsync = function sendAsync(opts, cb) {
  ...
  self.currentProvider.sendAsync(createPayload(opts, self.idCounter++), function (err, response) {
    ...
    if (err || responseObject.error) return cb(new Error('[vapjs-query] ' + (responseObject.error && 'rpc' || '') + ' error with payload ' + JSON.stringify(opts, null, 0) + ' ' + JSON.stringify(err || response.error, null, 0)));
    return cb(null, response.result);
  });
};

Object.keys(format.schema.methods).forEach(function (rpcMethodName) {
  Object.defineProperty(Vap.prototype, rpcMethodName.replace('eth_', ''), {
    enumerable: true,
